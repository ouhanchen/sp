...
char *host_to_ip(char *hostname, char *ip) { // 查出 host 對應的 ip
  ...
  printf("  ip=%s\n", ip);
  */
  inet_ntop(host->h_addrtype, host->h_addr_list[0], ip, SMAX);
  return ip;
}
...
int net_connect(net_t *net) {
    return connect(net->sock_fd, (struct sockaddr *)&net->serv_addr, sizeof(net->serv_addr));
}

int net_bind(net_t *net) {
    return bind(net->sock_fd, (struct sockaddr*)&net->serv_addr, sizeof(net->serv_addr));
}

int net_listen(net_t *net) {
    return listen(net->sock_fd, 10);
}

int net_accept(net_t *net) {
    return accept(net->sock_fd, (struct sockaddr*)NULL, NULL);
}
